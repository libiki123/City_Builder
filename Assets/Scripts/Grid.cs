...
public class Grid
{
    ...
    public static bool IsCellWakable(CellType cellType, bool aiAgent = false)       // if call from aiAgent we only want type road
    {
        if (aiAgent)
        {
    ...
    public List<Point> GetAdjacentCells(Point cell, bool isAgent)   // get all surround empty/road pos
    {
        return GetWakableAdjacentCells((int)cell.X, (int)cell.Y, isAgent);
    }

    public float GetCostOfEnteringCell(Point cell)      // Ex: high terran will have higher cost
    {
        return 1;
    }

    public List<Point> GetAllAdjacentCells(int x, int y)    // get the surround pos (left up right down)
    {
    ...
    {
        List<Point> adjacentCells = GetAllAdjacentCells(x, y);          // get surround pos
        for (int i = adjacentCells.Count - 1; i >= 0; i--)
        {
            if(IsCellWakable(_grid[adjacentCells[i].X, adjacentCells[i].Y], isAgent)==false)    // if not road or empty REMOVE it from list
            {
    ...
    public List<Point> GetAdjacentCellsOfType(int x, int y, CellType type)
    {
        ...
        for (int i = adjacentCells.Count - 1; i >= 0; i--)
        {
            if (_grid[adjacentCells[i].X, adjacentCells[i].Y] != type)      // remove if it not the same type
            {
                adjacentCells.RemoveAt(i);
            }
        }
